...
// The Supabase URL and publishable key are intentionally browser-safe. Never
// expose a Supabase secret or service-role key through this endpoint.
app.get("/api/community/config", (req, res) => {
  if (!hasConfigValue(SUPABASE_URL) || !hasConfigValue(SUPABASE_PUBLISHABLE_KEY)) {
    return res.status(503).json({
      error: "Supabase is not configured. Add SUPABASE_URL and SUPABASE_PUBLISHABLE_KEY to server/.env.",
    });
  }
...
app.get("/my-school",         (req, res) => res.sendFile(path.join(process.cwd(), "../client/my-school.html")));
